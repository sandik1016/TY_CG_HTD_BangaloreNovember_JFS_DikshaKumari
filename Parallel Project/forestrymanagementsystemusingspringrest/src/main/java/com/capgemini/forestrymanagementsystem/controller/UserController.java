...
import com.capgemini.forestrymanagementsystem.service.AdminService;
	...
	public class UserController {
		...
		@DeleteMapping(path="/remove-scheduler/{id}",produces = MediaType.APPLICATION_JSON_VALUE) //make it handler method and send it as path variabl
		public UserResponse removeScheduler(@PathVariable ("id")int id) {
		     UserResponse response = new UserResponse();
			if(service.removeScheduler(id))
			{
				response.setStatusCode(201);
				response.setMessage("success");
		...
		@DeleteMapping(path="/remove-client/{id}",produces = MediaType.APPLICATION_JSON_VALUE) //make it handler method and send it as path variabl
		public UserResponse removeClient(@PathVariable ("id")int id) {
		     UserResponse response = new UserResponse();
